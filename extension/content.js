...
// Opens a long-lived connection with the background process
const port = chrome.runtime.connect(window.localStorage.getItem(""))
...
const injectJSPayload = (payload, args = "") => {
  // Injects JS into the StackOverflow DOM
...
const createAnswerElement = () => {
  ...
  // TODO: Add SVG to lhsElement
  // TODO: Styling

  return textElement;
}

const populateAnswerElement = (answerElement, chatGPTOutput) =>
  chatGPTOutput.split(CODE_IDENTIFIER).forEach((text, index) => {
    if (index % 2) { // Code block
      const preElement = document.createElement("pre");
      const codeElement = document.createElement("code");

      preElement.className = "s-code-block";
 ...
 *****************/
 ...
 chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
   if (request.ping) {
     ...
     injectJSPayload(scrapeQuestion);
   }
 });

// Triggered when background script sends ChatGPT output
port.onMessage.addListener(message => {
  const { key, value } = message;
  if (key === "injectAnswer") {
    injectJSPayload(insertAnswer, value);
  }
});

// Listens for messages from injected payloads
window.addEventListener("message", event => {
  const { key } = event.data;

  if (key === "scrapedQuestion") {
